...
// Summary Modal
if(document.querySelector('[data-modal-summary]')) {

    function modalSummary() {
        const triggers = document.querySelectorAll('[data-modal-summary-trigger]');
        
        // Detect Closing Clicks
        const detectClosingClick = (event) => {
            if (!document.querySelector('[data-modal-summary-inner]').contains(event.target)) {
                closeModal();
            }
        };
        ...
        function openModal() {
            ...
            const tmln = gsap.timeline({});
            tmln
                .set('[data-modal-summary]', { autoAlpha: 1, overwrite: true }, 0)
                .to('[data-modal-summary-bg]', { opacity: 1, overwrite: true, duration: 1, ease: "power3.out"}, 0)
                .to('[data-modal-signup-gradients]', { scale:1, opacity: 1, overwrite: true, duration: 0.75, ease: "power3.inout"}, 0)
                .to('[data-modal-summary-inner]', { scale:1, opacity: 1, overwrite: true, duration: 1, ease: "elastic.out(1,0.6)"}, 0.125)
                .then(() => {
                    document.addEventListener('click', detectClosingClick);
                    document.querySelector('[data-modal-summary-close]').addEventListener('click', closeModal);
                    triggers.forEach(trigger => {
                        trigger.setAttribute('aria-expanded', 'true');
                    });
                });
        ...
        function closeModal() {
            ...
            const tmln = gsap.timeline({});
            tmln
                .to('[data-modal-summary-inner]', { scale:0.5, opacity: 0, overwrite: true, duration: 0.25, ease: "power3.out"}, 0)
                .to('[data-modal-summary-bg]', { opacity: 0, overwrite: true, duration: 0.5, ease: "power3.out"}, 0)
                .set('[data-modal-summary]', { autoAlpha: 0, overwrite: true }, 1)
                .then(() => {
                    document.removeEventListener('click', detectClosingClick);
                    document.querySelector('[data-modal-summary-close]').removeEventListener('click', closeModal);
                    triggers.forEach(trigger => {
                        trigger.setAttribute('aria-expanded', 'false');
                    });
                });
